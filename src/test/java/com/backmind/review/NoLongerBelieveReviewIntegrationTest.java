...
@Transactional
class NoLongerBelieveReviewIntegrationTest {
    ...
    @Test
    void recordsNoLongerBelieveFeedbackAndArchivesTheNote() throws Exception {
        mockMvc.perform(post("/api/notes/{id}/review", note.getId())
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"feedbackType": "NO_LONGER_BELIEVE"}
                                """))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("ARCHIVED"))
                .andExpect(jsonPath("$.beliefStatus").value("NO_LONGER_BELIEVE"));

        Note updatedNote = noteRepository.findByIdAndUserId(note.getId(), user.getId()).orElseThrow();
        assertThat(updatedNote.getStatus()).isEqualTo(NoteStatus.ARCHIVED);
        assertThat(updatedNote.getBeliefStatus()).isEqualTo(BeliefStatus.NO_LONGER_BELIEVE);

        assertThat(jdbcTemplate.queryForObject(
                """
                select count(*) from note_reviews
                where note_id = ? and user_id = ? and feedback_type = 'NO_LONGER_BELIEVE'
                """,
                Integer.class,
                note.getId(),
                user.getId()
