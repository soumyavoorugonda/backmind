...
@Transactional
class StillBelieveReviewIntegrationTest {
    ...
    @Test
    void recordsStillBelieveFeedbackAndAdvancesTheReviewInterval() throws Exception {
        mockMvc.perform(post("/api/notes/{id}/review", note.getId())
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"feedbackType": "STILL_BELIEVE"}
                                """))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.currentIntervalDays").value(3))
                .andExpect(jsonPath("$.beliefStatus").value("STILL_BELIEVE"))
                .andExpect(jsonPath("$.status").value("ACTIVE"));

        Note updatedNote = noteRepository.findByIdAndUserId(note.getId(), user.getId()).orElseThrow();
        assertThat(updatedNote.getBeliefStatus()).isEqualTo(BeliefStatus.STILL_BELIEVE);

        assertThat(jdbcTemplate.queryForObject(
                """
                select count(*) from note_reviews
                where note_id = ? and user_id = ? and feedback_type = 'STILL_BELIEVE'
                """,
                Integer.class,
                note.getId(),
                user.getId()
